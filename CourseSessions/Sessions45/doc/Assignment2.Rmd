...
Assignment 2 - Boats Segmentation: Cluster Analysis
========================================================

Anand Aiyer, Siddhartha Chaturvedi, Enrique Flores, Konstantin Zakharov, Demid Fedorov

Business Decisions
---------------------------------------------------------

Target each of the segements/clusters individually, recommendations are in the conclusion.
...
We decide to use a more data-oriented approach to finding, which variables to use. We picked the questions that had the largest weights for each factor that we created in Part 1 of the Boats exercise. We decided to use this method, as it was just as effective in finding the clusters we required when using relevant variables, and also it makes sense to take the relevant variable as clustering solutions -- too many variables used to cluster doesn't lead to effective cluster, especially if the distance function used is a regular Euclidean.

We decided to use the following variables,
...
**From repeated trial and error runs of verifying the clusters generated, we decided on 3 clusters**
...
We determined from the clusters that they can be categorized as:
 * Cluster 1: Wealth/Affluent Buyers
 * Cluster 2: Boating Enthusiasts
 * Cluster 3: Boating Hobbists
...
__The segments differ in terms of income, age, employment and is consistent between similar questions.__
...
We conclude that CreeqBoat needs to target these segements we determined seperately,

Here is what recommend,

 * Cluster 1: Wealth/Affluent Buyer - We target these buyers by seller boats that look good, but doesn't have many boating features. We do not need to give discounts as they are not price sensitive. We can give additional add-ons after the boats, such as servicing etc.
 * Cluster 2: Boating Enthusiasts - We can offer our high-end boats to these clients and give additional discounts or long-term memberships that allows them to buy our boats/products at a discount.
 * Cluster 3: Boating Hobbists - This is hobbists, they are price sensitive, we could try to attract them by giving one time discoubt offers.
